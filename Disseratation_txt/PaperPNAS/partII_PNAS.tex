...
This paper proposes an alternative paradigm: \emph{post-hoc direct retrieval with physics-informed supervision} (Figure~\ref{fig:data_levels}).
...
Two physics-injection mechanisms against these baselines: 
...
Combined prior-plus-regularization models were trained to test whether the mechanisms stack or interfere.
...
These two endmember classes perform well on the held-out validation split: the transformer baseline reaches F1-score $0.93$ for hematite ($N=729$) and $0.92$ for goethite ($N=141$), above the aggregate 95-class top-1 accuracy. 
The attention patterns show that the model emphasizes several wavelength neighborhoods physically plausible for ferric-mineral discrimination, but the learned peaks are not one-to-one matches to laboratory diagnostic wavelengths, as expected and explained further in the Section~\ref{sec:Discussion}.
...
The architectural-prior advantage is visible from the very first epoch (Figure~\ref{fig:earlyepoch}): at $H = 4$ the manual prior reaches $0.331$ top-1 after a single epoch versus $0.306$ for the unguided baseline, a $+2.5$~pp head start delivered by spectroscopic initialization alone. The LUSI configurations cluster at the bottom of the early-epoch top-1 ranking, consistent with the in-distribution penalty of statistical-invariance regularization.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{compare_runs_early.png}
\caption{First-five-epoch convergence trajectories across the ten parameter-matched configurations of Table~\ref{tab:crossscene}. \textbf{Left:} training loss (lower is better). \textbf{Center:} validation top-1. \textbf{Right:} validation top-3. Legend entries list the epoch-1 value and are sorted by epoch-1 validation top-1 (descending), so the early leader sits at the top. Colors are consistent with the full-trajectory view of the same runs. The 8-head configurations (PCA 8H, Trans 8H, Manual 8H, Manual + LUSI 8H) reach the highest epoch-1 top-1 ($\sim 0.34$--$0.36$), while at $H = 4$ the manual prior leads the unguided baseline at epoch 1 by $+2.5$~pp ($0.331$ vs $0.306$) --- the architectural-prior head start before any data-driven learning has had time to discover Fe$^{3+}$-relevant structure independently. The four LUSI configurations cluster near the bottom of the epoch-1 ranking, consistent with the consistency-loss training-cost overhead reducing effective per-epoch learning at saturated capacity.}
