...
\noindent
Mapping Earth's surface mineralogy from orbiting hyperspectral sensors is critical for understanding mineral dust's role in climate, but operational mineral products require computationally intensive atmospheric correction and spectral-library matching. 
We show that a cross-attention transformer trained on top-of-atmosphere reflectance can serve as a low-latency surrogate for mineral identification, using a single forward pass rather than an iterative retrieval workflow. 
The model provides an explicit interface for physics-informed learning: spectroscopic priors initialize attention biases, while statistical-invariance regularization tests label-preserving atmospheric perturbations. 
The result is a practical framework for physics-informed attention models that may support faster ground processing, onboard screening, and learning from upstream physical-model labels.
...
\section*{Abstract}
\noindent
NASA's Earth Surface Mineral Dust Source Investigation (EMIT) maps arid-region surface mineralogy with 285-band imaging spectroscopy, but operational mineral products require atmospheric correction and spectral-library matching before mineral identification. 
We present a cross-attention transformer that predicts Group~1 mineral labels directly from uncorrected top-of-atmosphere (TOA) reflectance, providing a post-hoc surrogate for the L1B-to-mineral-product mapping at inference time. 
...
Cross-region and full-granule experiments reveal a tradeoff: spectral priors improve interpretability and some rare-class behavior, whereas invariance regularization primarily affects robustness metrics rather than uniformly improving top-1 accuracy. 
...
\textbf{Keywords:} physics-informed deep learning $\vert$ cross-attention transformer $\vert$ hyperspectral imaging $\vert$ mineral classification $\vert$ statistical invariance $\vert$ spectral prior $\vert$ remote sensing 
...
EMIT mineral products are delivered with a two-month-plus average latency from acquisition \cite{emit_l2b_atbd}, reflecting downlink, ingest, queueing, atmospheric correction, mineral matching, quality control, and archive production.
For petabyte-scale missions or time-sensitive applications such as volcanic-ash mapping, post-fire debris characterization, and dust-storm response, this operational delivery cycle becomes a constraint separate from the cost of any single per-pixel computation.

We propose an alternative paradigm: \emph{post-hoc direct retrieval}.
Rather than executing the RTM-based atmospheric-correction and spectral-matching chain at inference time, a learned model is trained on previously processed L1B--L2B pairs and then used as a low-latency surrogate for the operational L1B-to-mineral-label mapping.
The model does not produce an atmospherically corrected reflectance product, nor does it claim independent mineral truth; its supervision is inherited from the operational science-data-system labels.
Its value is instead operational and methodological: after the one-time cost of generating training labels from a subset of processed granules, inference requires only a single forward pass from top-of-atmosphere (TOA) reflectance to mineral class.
...
We study this question through two physics-injection mechanisms: one architectural and one loss-based.

We present a cross-attention transformer that is physics-informed in the broad sense of theory-guided and physics-informed machine learning \cite{karpatne2017tgds, raissi2019pinn}.
...
Statistical-invariance knowledge enters through a consistency loss adapted from Vapnik and Izmailov \cite{vapnik2020}, which encourages predictions to remain stable under label-preserving TOA perturbations representing illumination scaling and smooth spectral-slope variation.
...
We demonstrate the approach on EMIT TOA hyperspectral observations for 95 Group~1 mineral classes.
...
Spectral-prior initialization improves interpretability and can improve accuracy or capacity efficiency depending on head count and prior construction, while statistical-invariance regularization exposes a robustness tradeoff rather than uniformly improving top-1 accuracy.
Together, these results position physics-informed attention as a practical mechanism for accelerating, interrogating, and stress-testing hyperspectral mineral-identification pipelines trained from upstream physical-model products.
...
\subsection{Cross-attention transformer learns Fe\texorpdfstring{$^{3+}$}{3+}-consistent spectral structure from TOA reflectance}

The architecture in Figure~\ref{fig:architecture} is a cross-attention transformer with $H$ learned query vectors that probe an $L=285$-band spectral token sequence. 
Each head produces attention weights $\mathbf{A}_h \in \R^L$, which provide a useful diagnostic of the wavelength regions emphasized by the model. 
We first trained a transformer baseline on 1.26~million class-balanced TOA pixels from 396 EMIT granules over Africa and the Middle East, using derivative tokenization, learned query vectors, and a zero-initialized \emph{trainable} attention bias $\bm{\beta}_h$. 
...
The two physics-injection mechanisms augment this base pass rather than replacing it. 
Spectral priors enter through the per-head attention bias $\bm{\beta}_h$, while statistical-invariance regularization enters through an auxiliary consistency-loss branch.}
\label{fig:architecture}
\end{figure}

Figure~\ref{fig:trans4_attn} compares class-conditional mean attention for hematite and goethite at $H=4$ across four parameter-matched configurations: transformer-only, manual spectral prior, PCA-curated prior, and LUSI-only. 
...
This is expected because the model operates on TOA reflectance rather than laboratory surface reflectance, and because derivative tokenization emphasizes local slope and curvature as well as band magnitude.
...
Curves compare the transformer baseline, manual Fe$^{3+}$ prior, PCA-curated prior, and LUSI-only configuration. 
...
The unguided transformer and LUSI-only configurations retain substantial attention at non-canonical wavelengths, indicating that attention diagnostics should be interpreted as supporting evidence rather than causal proof of mineral discrimination.}
...
At $H=4$, the manual prior produces statistically significant head-averaged Spearman correlation with absorption depth for both hematite ($\rho=0.238$, $p<10^{-3}$) and goethite ($\rho=0.202$, $p<10^{-2}$). 
The curated PCA prior also produces positive alignment ($\rho=0.216$ for hematite and $\rho=0.170$ for goethite). 
The strongest per-head cases occur when the prior anchors attention near known ferric-iron regions: manual-prior Head~1 reaches $\rho=0.389$ ($p<10^{-8}$) for hematite, with peaks near 530/544/559~nm and 873~nm, while manual-prior Head~0 reaches $\rho=0.394$ ($p<10^{-7}$) for goethite, with peaks near 470/485~nm and 902~nm. 
In contrast, the unguided $H=4$ transformer and LUSI-only models do not show significant head-averaged alignment for these minerals. 
At $H=8$, however, the unguided transformer reaches $\rho=0.320$ ($p<10^{-6}$) for hematite, showing that sufficient attention capacity can discover Fe$^{3+}$-consistent spectral structure without explicit initialization.
...
Discrimination also depends on the value vectors $\mathbf{V}_j$, which encode the reflectance, derivative, and band-position content at each wavelength. 
Thus, attention identifies where the model samples spectral evidence, while the downstream classifier separates minerals using both attention magnitudes and the wavelength-dependent content being pooled.
...
We evaluated two spectroscopic initializations. 
...
The LUSI-inspired consistency loss penalizes prediction drift under two label-preserving transformations of the input spectrum: multiplicative scaling, representing illumination variation, and a smooth additive spectral tilt, representing low-order atmospheric or scattering-related slope variation.
...
The scale and tilt transformations capture only low-order label-preserving effects and do not represent the full atmospheric radiative-transfer process. 
...
The default manual prior uses the visible and near-infrared ferric-iron regions
\[
480,\ 535,\ 670,\ 860,\ 920~\mathrm{nm},
\]
...
The first transformation is a multiplicative scale factor
\[
\rho \sim U(0.8,1.2),
\]
representing moderate illumination or topographic-brightness variation. 
The second is a smooth additive spectral tilt
\[
\xi \sim \mathcal{N}(0,0.01),
\]
...
D_{\mathrm{KL}}
\left(
p_{\theta}(\mathbf{x})^{\mathrm{sg}}
\;\|\;
p_{\theta}(T(\mathbf{x}))
\right),
\]
where $T(\cdot)$ denotes the sampled transformation and superscript $\mathrm{sg}$ indicates stop-gradient. 
...
To reduce dominance by the most common classes, we apply a stratified per-class cap of 7{,}500 pixels and then split the sampled dataset 90/10 into training and validation sets using a fixed random seed. 
...
We report top-1 accuracy, top-3 accuracy, macro-F1, and macro-recall. 
