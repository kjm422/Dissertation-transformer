...
A 4 and 8-head transformer baseline are trained on 1.26~million class-balanced TOA pixels from 396 EMIT granules over Africa and the Middle East.
No spectroscopic prior and no statistical-invariance regularization were applied in this baseline. 
Two physics-informed configurations are subsequently evaluated against this transformer-only baseline: 1) spectroscopic priors, implemented as per-head attention-bias initialization (from diagnostic wavelengths or USGS reference spectra), and 2) statistical-invariance regularization, implemented as a consistency loss (under label-preserving illumination and spectral-slope perturbations). 
