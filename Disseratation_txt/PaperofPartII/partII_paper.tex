...
A 4-head transformer initialized with the manual spectral prior (top-1 = 0.808, train+val 117 min) reaches accuracy statistically indistinguishable from the 8-head transformer-only baseline (top-1 = 0.813, 136 min) at 14\% lower wall-clock training time --- demonstrating that the literature-distilled prior substitutes for one architectural doubling. The full ablation table, including PCA-curated (top-1 = 0.809) and LUSI variants and head-counts $H = 4, 8$, is reported in the dissertation.
...
\paragraph{Early-epoch acceleration: architectural vs.\ loss-based priors.}
Final accuracy is only one signature of the prior's contribution; the rate of convergence is another.
Figure~\ref{fig:earlyepoch} compares the first five epochs of training across four $H = 4$ configurations: transformer-only, manual prior, PCA-curated (\texttt{zabs}) prior, and LUSI-only.
The manual prior leads on every logged metric: top-1 accuracy at epoch 1 is $0.331$ with the manual prior versus $0.305$ for transformer-only --- an immediate $+2.6$~percentage-point lead from initialization alone --- and the gap remains $\sim$2--3~pp through epoch 5.
The PCA-curated \texttt{zabs} prior shows a smaller but consistent push: epoch-1 top-1 is $0.313$ ($+0.8$~pp over transformer-only), and its training-loss trajectory tracks the manual prior more closely than transformer-only.
By contrast, the loss-based prior alone (LUSI-only at $H = 4$) is \emph{slower} than transformer-only in early-epoch convergence --- consistent with its in-distribution accuracy gap (top-1 = $0.773$ vs.\ $0.791$) and with PINN-literature observations that physics-driven loss regularization can trade training-set fit for extrapolation \cite{raissi2019pinn, karpatne2017tgds}.
The asymmetry between architectural and loss-based priors during early training reinforces the two-mechanism reading of the framework: architectural priors provide an immediate inductive head start, while loss-based priors invest training-set fit in robustness that pays out at test time on out-of-distribution scenes (cross-region generalization, reported in the dissertation).

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{train_top1_top3.png}
\caption{First-five-epoch convergence trajectories at $H = 4$ for four configurations: transformer-only, manual prior, PCA-curated (\texttt{zabs}) prior, and LUSI-only. \textbf{Left:} training cross-entropy loss; \textbf{center:} validation top-1; \textbf{right:} validation top-3. The legend reports the epoch-1 value of each metric for each configuration. The manual prior leads every metric across all five epochs ($+2.6$~pp top-1 by epoch 1); the PCA-curated \texttt{zabs} prior provides a smaller but consistent push over the transformer-only baseline ($+0.8$~pp); LUSI alone (no architectural prior) converges \emph{more slowly} than transformer-only in-distribution, consistent with its in-distribution accuracy gap and with PINN-literature observations that physics-driven loss regularization trades training-set fit for extrapolation.}
