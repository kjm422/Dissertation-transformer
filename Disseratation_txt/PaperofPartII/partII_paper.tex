...
This Perceiver-style cross-attention principle is adopted here for spectral pooling. 
...
The pooled vector $\mathbf{z}_{\text{cat}}$ is mapped to per-class logits by a two-layer feed-forward network with ReLU activations and dropout $p = 0.1$, compressing the $Hd = 1{,}536$-dimensional multi-head representation to $d = 192$ before producing $Q_1 = 95$ class logits (Figure~\ref{fig:architecture}); the predicted mineral ID $\hat{q}_{1}$ is the argmax over those logits. The softmax distribution $\hat{\mathbf{p}}$ over $Q_1 = 95$ classes is a \emph{model} probability rather than a calibrated estimate of real-world class probabilities; top-1 and top-3 retrievals are reported as relative measures, and any frequentist interpretation is treated as requiring separate calibration.
...
For each mineral $c$ and configuration, Spearman rank correlation is computed between the attention vector and the absorption depth $1 - \mathbf{w}_{\text{ref}}^{(c)}$ derived from the corresponding USGS reference spectrum. The metric tests whether the model's attention magnitudes rank wavelengths in the same order as the mineral's actual absorption pattern. Three patterns emerge:
...
\paragraph{Epoch-1 architectural-prior head start.}
Figure~\ref{fig:earlyepoch_zoom} zooms in on the first 5 epochs across all 10 parameter-matched configurations of Table~\ref{tab:ablation} (including the two Manual + LUSI runs). At $H = 4$, the manual prior reaches $0.331$ top-1 after a single epoch versus $0.306$ for the unguided baseline --- a $+2.5$~pp head start delivered by spectroscopic initialization before any data-driven learning has had time to discover Fe$^{3+}$-relevant structure independently. At $H = 8$, the four configurations (PCA, Trans, Manual, Manual + LUSI) cluster at the top of the epoch-1 ranking ($\sim 0.34$--$0.36$), with capacity rather than prior identity carrying most of the early-epoch advantage at this scale. The LUSI configurations cluster near the bottom of the epoch-1 top-1 ranking, consistent with the consistency-loss training-cost overhead reducing effective per-epoch learning.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{compare_runs_early.png}
\caption{First-five-epoch convergence trajectories across the ten parameter-matched configurations of Table~\ref{tab:ablation}. \textbf{Left:} training loss. \textbf{Center:} validation top-1. \textbf{Right:} validation top-3. Legend entries list the epoch-1 value and are sorted by epoch-1 validation top-1 (descending). Colors are consistent with the full-trajectory view of the same runs in Figure~\ref{fig:earlyepoch}. The manual prior leads the unguided baseline at $H = 4$ by $+2.5$~pp after the first epoch ($0.331$ vs $0.306$); the four LUSI configurations cluster near the bottom of the epoch-1 ranking.}
...
The Manual + LUSI rows of Table~\ref{tab:ablation} were added in a 2026-05 follow-up run; the convergence-trajectories figure (Figure~\ref{fig:earlyepoch}) was generated from the original nine-configuration set and does not include those trajectories.
...
\section{Comparison with a non-attention random-forest baseline}
\label{sec:rf_baseline_comparison}

Random forest classifiers are widely used for hyperspectral pixel classification~\cite{wang2022hyspex} and frequently appear as a baseline in transformer-based comparison studies~\cite{hong2021spectralformer}, providing a natural reference point for asking whether the cross-attention transformer's added architectural complexity translates into measurable accuracy gains over a classical model that treats each spectral band independently. A 500-tree random forest with $\sqrt{L} \approx 17$ random features per split, \texttt{min\_samples\_split = 10}, and \texttt{min\_samples\_leaf = 4} was trained on the identical $\sim$$1.03$~million-pixel training split (Section~\ref{sec:training_data}). The trained classifier was then evaluated at three deployment scales matching the transformer evaluation: held-out pixel-level validation pool (Section~\ref{sec:trans_only_results}), full-granule inference on three representative Africa/Middle East granules (Section~\ref{sec:full_image_inference}), and cross-region deployment on 20 SW US granules (Table~\ref{tab:crossscene}).

\begin{table}[!htbp]
\centering
\small
\caption{Random-forest baseline vs.\ cross-attention transformer configurations at three deployment scales. Transformer ranges span the eight Trans/Manual/PCA-zabs/LUSI configurations at $H = 4$ and $H = 8$ from Tables~\ref{tab:ablation} and~\ref{tab:crossscene}. Per-granule inference time is wall-clock on a single Apple M4 Max laptop.}
\label{tab:rf_compare}
\begin{tabular}{p{0.42\linewidth}p{0.18\linewidth}p{0.28\linewidth}}
\hline
\rowcolor[gray]{0.85}
\textbf{Metric} & \textbf{RF} & \textbf{Transformer range} \\
\hline
In-distribution validation top-1 (pixel)         & $0.716$        & $0.789$--$0.819$ \\
\hline
In-distribution validation top-3 (pixel)         & $0.922$        & $0.961$--$0.970$ \\
\hline
Granule top-1 (Africa, mean of 3 granules)       & $0.769$        & $0.874$--$0.892$ \\
\hline
Granule top-1 (SW US, mean of 20 granules)       & $0.561$        & $0.634$--$0.724$ \\
\hline
Granule SW US top-3                              & $0.799$        & $0.875$--$0.919$ \\
\hline
Granule SW US macro recall                       & $0.091$        & $0.171$--$0.207$ \\
\hline
Inference time per granule                       & $\sim$$105$~s  & $\sim$$45$~s     \\
\hline
\end{tabular}
\end{table}

The accuracy gap between the random forest and the transformer configurations is present at pixel level ($-7.3$ to $-10.3$~pp top-1) and widens with deployment scale: to $\sim$$10$--$12$~pp at Africa granule scale and to $7$--$16$~pp at SW US cross-region scale. The top-3 gap is the smallest at granule scale (RF $= 0.976$ vs transformer $0.996$--$0.998$, only $\sim$$2$~pp), indicating that the random forest knows the correct mineral is in its top three predictions almost as often as the transformer does. The top-3 gap widens again under cross-region deployment (RF $= 0.799$ vs transformer $0.875$--$0.919$).

The most striking gap is macro recall on SW US, where RF reaches $0.091$, less than half of every transformer configuration ($\geq 0.171$). This is consistent with RF's tree splits each thresholding a single spectral band at a time, in contrast to cross-attention's softmax weighting across all $285$ bands jointly: rare-mineral discrimination often depends on the joint shape of multiple bands (a derivative-spectrum minimum at one wavelength relative to a continuum elsewhere, or a band-ratio between two diagnostic regions), and a per-band classifier cannot directly represent such cross-band features. The result is a model that has learned per-band thresholds well enough to land the correct answer in the top three nearly $80\%$ of the time even cross-region, but cannot recover the rare Group~1 classes that the cross-attention transformer captures jointly across wavelengths.

RF inference on a single $1.6$-million-pixel granule takes $\sim$$105$~seconds on the same Apple M4 Max laptop used for the transformer runs, $\sim$$2.3\times$ slower per granule than the transformer's $\sim$$45$~seconds, despite a substantially lower one-time training cost ($\sim$$22$~minutes for RF versus $117$--$326$~minutes for the transformer configurations). At deployment scale --- where the trained model is repeatedly applied across many granules --- the transformer's per-granule inference advantage compounds; the higher upfront training cost is amortized across the larger granule volume the model is asked to classify after training.

\section{Discussion and conclusions}
\label{sec:conclusions}

The central result of this work is not that every physics-informed intervention improves every metric.
The experiments show that cross-attention architectures provide a controlled way to test \emph{how} physical knowledge enters a learned remote-sensing surrogate --- whether through structured initialization of an explicit attention bias (spectroscopic priors) or through label-preserving augmentations enforced by a consistency loss (LUSI-like regularization) --- and reveal under what conditions that knowledge helps, is neutral, or actively constrains the model.
...
They indicate \emph{where} each learned query samples the spectrum, while the final class decision also depends on the value vectors $\mathbf{V}_j$, which encode reflectance magnitude, local derivative structure, and band-position information.
This distinction is particularly important for hematite and goethite, whose diagnostic features overlap substantially: both minerals share attention peaks, which is not a failure of discrimination but a reflection of shared ferric-iron spectral structure.
...
The LUSI-inspired consistency loss was designed to encourage stability under illumination scaling and smooth spectral-slope perturbations, both acknowledged sources of label-quality degradation in the operational EMIT pipeline~\cite{emit_l2a_atbd, emit_l2b_atbd}.
The physical motivation comes from the first-order radiative-transfer decomposition linking TOA reflectance to surface reflectance (Equation~\ref{eq:toa_decomp}): the two LUSI predicates approximate label-preserving directions in TOA-input space derived from this decomposition --- the multiplicative scale perturbation approximates broadband changes in $T_{\downarrow}T_{\uparrow}$, and the smooth additive spectral-slope perturbation approximates a low-order change in $\rho_{\mathrm{path}}$.
The implementation diverges from Vapnik and Izmailov's original LUSI formulation~\cite{vapnik2020}, in which predicates and statistical invariants enter as \emph{hard constraints} on the admissible function class via minimization of a constrained functional in a Reproducing Kernel Hilbert Space (RKHS).
The neural training setting does not easily admit such hard constraints, so the predicates here enter softly, as a teacher--student consistency loss between predictions on the original and perturbed spectra.
The empirical results show that this soft approximation does not improve in-distribution top-1 accuracy and concentrates its benefit in rare-class macro recall on out-of-distribution scenes (Section~\ref{sec:cross_region}): at $H = 4$, LUSI exceeds the unguided baseline's macro recall on $19$ of $20$ SW US granules, even while losing to both Trans 4H and Manual 4H on top-1.
Exploring the formal hard-constraint implementation is a natural next step (Section~\ref{sec:future_work}) that may either widen the macro-recall gain or eliminate the in-distribution accuracy cost.

\paragraph{Spectral priors and the LUSI-inspired loss do not stack additively.}
At $H = 4$, combining either spectral prior with the LUSI-inspired loss is consistently worse than the prior alone on every metric: PCA-curated + LUSI reaches top-1 $0.790$ ($-1.9$~pp vs PCA-curated alone), and Manual + LUSI reaches $0.795$ ($-1.3$~pp vs Manual alone).
At $H = 8$ the conflict softens to neutrality.
The two mechanisms target different performance axes: spectral priors improve in-distribution accuracy, interpretability, and rare-class capture (especially at constrained capacity), whereas the LUSI-inspired loss's effect is concentrated in rare-class macro recall on out-of-distribution scenes.
The bias-amplitude diagnostics suggest that the consistency loss uses the trainable attention bias as a buffer between augmented and unaugmented passes, distorting the structured spectral initialization.

\paragraph{Post-hoc direct retrieval as a reliable operational surrogate.}
The proposed model should be understood as a post-hoc direct retrieval surrogate for the operational L1B-to-mineral-product mapping, not as an independent physical retrieval of surface mineralogy.
Its labels are inherited from the EMIT science data system, and its accuracy is therefore bounded by the quality, thresholds, and assumptions of the upstream atmospheric-correction and spectral-matching pipeline rather than by any independent ground truth.
Within that scope, the operational advantage is substantial: once trained, the model maps TOA reflectance to mineral labels using a single forward pass and does not invoke an RTM at inference time.
Full-granule experiments show that this inference can be performed on commodity Apple Silicon hardware in approximately $45$~seconds per $1.6$-million-pixel granule (Section~\ref{sec:full_image_inference}), and $\sim$$2.3\times$ faster than the random-forest baseline at the same scale (Section~\ref{sec:rf_baseline_comparison}).
A fast surrogate supports rapid ground screening, onboard event detection, downlink prioritization, or exploratory analyses in which approximate mineral labels are useful before validated products are available.
...
In hyperspectral remote sensing, the feature axis is wavelength and the priors come from spectroscopy; in other domains, the analogous axis might be time, frequency, spatial scale, molecular position, or graph connectivity.
This makes the approach most appropriate for problems where labels are expensive (because they are generated by an upstream physical model or expert pipeline) and where domain knowledge specifies diagnostic input regions or label-preserving transformations.
...
Fourth, the LUSI-inspired perturbations capture only low-order label-preserving effects (multiplicative brightness, smooth slope) and do not represent the full atmospheric radiative-transfer process.
...
A second direction is causal interpretability of the attention patterns: band-masking, counterfactual spectral editing, and attention-bias intervention experiments could determine whether the wavelengths highlighted by attention are \emph{necessary} for correct classification, moving the attention story from \emph{consistent with} known spectroscopy to a stronger causal claim.

A third direction is uncertainty quantification, including temperature scaling, conformal prediction, ensembles, and post-hoc calibration against operational confidence measures.
Since the softmax outputs are model probabilities rather than calibrated posterior probabilities, deployment would require calibration or uncertainty quantification to identify when the surrogate should defer to the operational pipeline.

A fourth direction is adaptive physics infusion: instead of fixing manual or PCA priors at training, the model could learn when to trust, attenuate, or override a prior based on scene conditions (e.g., AOD-conditioned prior strength, or per-pixel prior gating).

A fifth direction is to explore the formal LUSI implementation as a constrained functional on a Reproducing Kernel Hilbert Space (RKHS), or other constrained-optimization approximations that more faithfully realize Vapnik and Izmailov's hard-constraint setup~\cite{vapnik2020}.
Given that even the soft consistency-loss approximation used here exposes a measurable robustness-vs-accuracy axis at the $\sim$$1.26$-million-pixel training scale, the formal implementation may either widen the macro-recall gain on cross-region deployment or eliminate the in-distribution accuracy cost.

Finally, onboard or near-realtime deployment studies should evaluate not only inference speed but also memory footprint, radiation-tolerant compute constraints, downlink prioritization value, and failure-detection logic for when the surrogate should defer to full ground processing.
