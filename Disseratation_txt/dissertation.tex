...
The architecture of Chapter~\ref{chap:transformer} is first evaluated with no physics-informed prior and no LUSI regularization ($\bm{\beta}_h = \mathbf{0}$, frozen) to establish the baseline against which the physics-informed (Section~\ref{chap:physics}) and LUSI (Section~\ref{chap:lusi}) variants are compared in Chapter~\ref{chap:ablation}.
...
Doubling the number of heads from 4 to 8 yields a +2.2 percentage-point gain in top-1 accuracy and +0.6 in top-3 at the cost of a 16\% increase in training time, motivating $H=8$ as the canonical configuration for the physics-informed extensions evaluated in Chapter~\ref{chap:ablation}.
All wall-clock timings reported throughout this dissertation were measured on a single Apple M4 Max laptop (40-core GPU) using PyTorch's MPS backend.
The absolute training+validation budgets therefore reflect what a single consumer workstation can deliver, not an HPC environment.
...
Figure~\ref{fig:trans4_attn} shows class-conditional mean per-head attention weights for the $H=4$ transformer-only baseline for hematite (class~29) and goethite (class~2).
...
The transformer-only model recovers physically plausible attention regions for hematite and goethite without any guidance, but the alignment with literature diagnostic wavelengths is partial: some heads lock onto continuum or atmospheric-window features rather than known Fe$^{3+}$ absorption bands.
The next chapter explores whether explicitly biasing the attention initialization toward USGS-derived spectral variance (Section~\ref{chap:physics}) or enforcing label-preserving invariance through LUSI consistency regularization (Section~\ref{chap:lusi}) shifts the learned attention toward the canonical diagnostics and whether either change affects classification accuracy.
...
The attention bias $\bm{\beta}_h$ is initialized from Principal Component Analysis on $M = 93$ USGS Spectral Library Group~1 reference spectra \cite{kokaly2017usgs}, each convolved to EMIT’s $L = 285$ bands. Note that $M < Q_1$ because not all 95 mineral classes have a distinct laboratory reference spectrum (some classes are variants or mixtures). These spectra are drawn from the same class of laboratory reference libraries used by the EMIT/Tetracorder mineral-identification pipeline, which matches continuum-normalized observed spectra to continuum-normalized library references via a fit-score and feature-depth criterion \cite{clark2003tetracorder, clark2024tetracorder, emit_l2b_atbd}. Where Tetracorder encodes spectral knowledge through hand-crafted feature-matching rules applied to surface reflectance, the PCA spectral priors distill the reference library into a statistical summary, the principal axes of spectral variance, that biases attention toward wavelengths where the laboratory library exhibits substantial mineralogical variability.
...
% TODO: Plot USGS surface reflectance for hematite alongside EMIT TOA reflectance for hematite-labeled pixels on the same axes, to visually demonstrate how diagnostic features are preserved through the atmosphere.
% TODO: Repeat PCA analysis using only the 10 EMIT target minerals instead of the full 93-spectrum library to assess whether a focused reference set produces more discriminative priors.
...
\centering
\small
\caption{Ablation: transformer-only baseline versus the two architectural-prior constructions, at $H = 4$ and $H = 8$. All prior runs use $\alpha = 1.0$ and a 3-epoch freeze of the prior-bearing heads. Wall-clock timings are for the full 120-epoch training+validation run on a single Apple M4 Max GPU (40-core) using PyTorch's MPS backend; absolute numbers reflect what a single consumer workstation can deliver. Legacy in-line-PCA and LUSI configurations are discussed in Section~\ref{sec:pca_legacy_caveat}.}
\label{tab:ablation}
...
Two architectural-prior constructions at $H=4$ --- the manual prior (literature-distilled Fe$^{3+}$ wavelengths) and the PCA-curated prior --- reach top-1 accuracies of $0.808$ and $0.809$ respectively, statistically indistinguishable from each other and from the 8-head transformer-only baseline ($0.813$) at $\sim$14\% lower wall-clock training time (117--124 vs.\ 136 minutes). The convergence of two independently constructed priors at essentially the same final accuracy supports an interpretation in which the architectural prior contributes \emph{capacity substitution} rather than \emph{wavelength selection}: the model benefits from any spectroscopically structured initialization at $H = 4$, not from a specific choice of canonical bands. Operationally, pinning a small set of spectroscopically informed wavelengths into the attention bias substitutes for one architectural doubling. (A third construction --- PCA-curated with max normalization rather than z-score-of-$|v|$ --- was also evaluated and reached the same $\sim 0.809$ top-1; it is omitted from Table~\ref{tab:ablation} because the z-score normalization mathematically subsumes it as the legacy variant carried forward from the in-line pipeline of Section~\ref{sec:pca_legacy_caveat}.)

The prior contributes a $+1.7$ to $+1.8$~percentage-point top-1 improvement over transformer-only at $H = 4$ ($0.791 \to 0.808$ for the manual prior; $0.791 \to 0.809$ for the PCA-curated zabs prior) but only $-0.7$~pp at $H = 8$ ($0.813 \to 0.806$). This is consistent with the standard observation that physics-informed priors help most when the model's capacity is constrained \cite{raissi2019pinn, karpatne2017tgds}: at $H = 8$ the model already discovers most of the diagnostic regions on its own (Figure~\ref{fig:trans4_attn} shows three of four heads doing so without any guidance at $H = 4$, and the picture is cleaner at $H = 8$), so the prior contributes nothing on top.
...
\paragraph{Early-epoch acceleration: architectural and loss-based priors behave asymmetrically.}
Final accuracy is only one signature of the prior's contribution; the rate of convergence is another. Figure~\ref{fig:earlyepoch} compares the first five epochs of training across four $H = 4$ configurations: transformer-only, manual prior, PCA-curated zabs, and LUSI-only.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{train_top1_top3.png}
\caption{First-five-epoch convergence trajectories at $H = 4$ for transformer-only, manual prior, PCA-curated zabs, and LUSI-only. Left: training cross-entropy loss; center: validation top-1; right: validation top-3. Legend reports the epoch-1 value for each configuration. The manual prior leads on every metric across all five epochs; the PCA-curated zabs prior provides a smaller but consistent push over transformer-only; LUSI alone (no architectural prior) converges \emph{more slowly} than transformer-only in-distribution, consistent with its in-distribution accuracy gap and with PINN-literature observations that physics-driven loss regularization trades training-set fit for extrapolation.}
\label{fig:earlyepoch}
\end{figure}

The manual prior leads on every logged metric: top-1 accuracy at epoch 1 is $0.331$ with the manual prior versus $0.305$ for transformer-only --- an immediate $+2.6$~pp lead from initialization alone --- and the gap remains $\sim$2--3~pp through epoch 5. The PCA-curated zabs prior shows a smaller but consistent push: epoch-1 top-1 is $0.313$ ($+0.8$~pp over transformer-only), and the training-loss trajectory tracks the manual prior more closely than transformer-only. By contrast, the loss-based prior alone (LUSI only at $H = 4$) is \emph{slower} than transformer-only in early-epoch convergence --- consistent with its in-distribution accuracy gap ($0.773$ vs.\ transformer-only $0.791$) and with PINN-literature observations that physics-driven loss regularization can trade training-set fit for extrapolation. The asymmetry between architectural and loss-based priors during early training reinforces the two-mechanism reading of the framework: architectural priors provide an immediate inductive head start, while loss-based priors invest training-set fit in robustness that pays out at test time on out-of-distribution scenes (Chapter~\ref{chap:generalization}).
...
\begin{enumerate}
  \item \emph{Three independent prior constructions converge.} Three architectural priors at $H=4$ (manual literature wavelengths; PCA-curated zabs; PCA-curated max) all reach top-1 accuracy in the range $0.808$--$0.809$, statistically indistinguishable from the 8-head transformer-only baseline ($0.813$) at $\sim$14\% lower training time. The convergence supports an interpretation in which the architectural prior contributes \emph{capacity substitution} rather than \emph{wavelength selection}.
