...
\usepackage{xcolor}           % For TODO markers
...
In contrast to in-line emulators like sRTMnet, which accelerate the forward model $F$ within this iterative loop, this dissertation proposes a \emph{post-hoc RTM-bypass} that skips both the forward model and the iterative retrieval entirely, after training on a small set of measurement-mineral pairs.
...
\begin{enumerate}
  ...
  \item \textbf{Interpretability:} Because each head produces a single attention vector over bands (not a band-to-band attention matrix), the weights $\mathbf{A}_h$ directly indicate which wavelengths each head considers important. This is what enables the per-head spectroscopic validation in Chapter~\ref{chap:ablation} --- an interpretability advantage that would not exist with standard self-attention.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Physics injection interface:} The attention bias $\bm{\beta}_h$ provides a clean mechanism for incorporating domain knowledge without changing the architecture --- a capability absent from DETR, Perceiver, and all HSI transformers in the literature.
...
Together, these capture the morphology of absorption features (depth, asymmetry, width) as local features, analogous to what a CNN backbone extracts in DETR before the transformer evaluates global context. 
...
For example, one head may focus on Fe$^{3+}$ charge transfer bands ($\sim$530~nm) while another attends to crystal field transitions ($\sim$860~nm).
...
This construction is related to, but distinct from, prior uses of PCA and attention in hyperspectral learning. For example, Wang et al.~\cite{wang2021attention_pca_guided} use PCA-guided self-supervised feature extraction within an attention-based spatial--spectral network for hyperspectral change detection. In contrast, the approach here does not use PCA to reduce the EMIT observations or to define an auxiliary self-supervised target. Instead, PCA is applied to an external, sensor-convolved mineral reference library, and the resulting absolute loadings are used to initialize an additive pre-softmax attention bias. The contribution is therefore a reference-library-derived spectral prior: a soft, trainable initialization that anchors selected attention heads to mineralogically meaningful wavelength regions while allowing supervised training on TOA reflectance to refine or override the prior.
