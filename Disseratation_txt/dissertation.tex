...
The transformer addresses the limitations identified in Section~\ref{sec:part1_limitations}:
\begin{itemize}
  \item \textbf{Spectral interpretability:} Attention weights $A_{h,j}$ provide per-band, per-head, per-class importance scores as a direct output of the forward pass.
  \item \textbf{Global spectral context:} Unlike 1D-CNNs with local receptive fields, cross-attention evaluates all 285 bands simultaneously, enabling long-range spectral dependencies.
  \item \textbf{Linear complexity:} Using learned queries rather than self-attention reduces complexity from $O(L^2)$ to $O(H \times L)$, where $H$ is the number of attention heads and $L = 285$ is the number of bands.
  \item \textbf{Physics injection interface:} The attention bias $\boldsymbol{\beta}_{h,j}$ provides a clean mechanism for incorporating domain knowledge without changing the architecture.
\end{itemize}
...
The input embeddings are projected into Key and Value spaces ($\mathbf{K} = \mathbf{E}_{final}\mathbf{W}_K$, $\mathbf{V} = \mathbf{E}_{final}\mathbf{W}_V$). For $H$ attention heads with learnable query vectors $\mathbf{q}_h \in \R^d$:
\begin{align}
