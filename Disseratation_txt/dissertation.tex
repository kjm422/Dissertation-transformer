...
\centering
\small
\caption{Complete ablation results (120 epochs, $\sim$1.26M training pixels). Manual-prior rows use the canonical 5-band Fe$^{3+}$ list of Table~\ref{tab:fe_diagnostics} (480/535/670/860/920~nm) with round-robin band-to-head assignment, max normalization, $\alpha = 1.0$, and a 3-epoch freeze. PCA-prior rows used the legacy in-line PCA pipeline whose bad-band-mask issue is discussed in Section~\ref{sec:pca_legacy_caveat}.}
\label{tab:ablation}
\begin{tabular}{p{0.30\textwidth}p{0.16\textwidth}p{0.10\textwidth}p{0.10\textwidth}p{0.14\textwidth}}
\hline
\rowcolor[gray]{0.85}
\textbf{Configuration} & \textbf{Heads (prior/total)} & \textbf{Top-1} & \textbf{Top-3} & \textbf{Time (min)} \\
\hline
Transformer only & 4 & 0.791 & 0.962 & 117 \\
\hline
\textbf{Manual prior} & \textbf{4/4} & \textbf{0.809} & \textbf{0.967} & \textbf{117} \\
\hline
\textbf{Transformer only} & \textbf{8} & \textbf{0.813} & \textbf{0.968} & \textbf{136} \\
\hline
Manual prior & 5/8 & 0.806 & 0.966 & 125 \\
\hline
PCA (continuum, legacy) & 4/4 & 0.804 & 0.966 & 141 \\
\hline
PCA (no continuum, legacy) & 4/4 & 0.810 & 0.967 & --- \\
\hline
PCA (continuum, legacy) & 4/8 & 0.806 & 0.966 & 127 \\
\hline
PCA (no continuum, legacy) & 8/8 & 0.821 & 0.971 & --- \\
\hline
LUSI only & 4 & 0.787 & 0.961 & 269 \\
\hline
LUSI only & 8 & 0.796 & 0.963 & 293 \\
\hline
PCA + LUSI (legacy) & 4/8 & 0.805 & 0.965 & 311 \\
\hline
\end{tabular}
\end{table}

\paragraph{Headline finding: manual prior at $H=4$ matches transformer-only at $H=8$.}
The most useful comparison in Table~\ref{tab:ablation} is between the manual-prior $H=4$ row (top-1~$=0.809$) and the transformer-only $H=8$ row (top-1~$=0.813$). The 4-head configuration with a literature-distilled spectral prior reaches accuracy statistically indistinguishable from the 8-head transformer-only baseline at 14\% lower wall-clock training time (117 vs.\ 136 minutes). Operationally, this means that pinning five canonical Fe$^{3+}$ wavelengths into the attention bias substitutes for one architectural doubling: instead of paying for an additional set of $H = 4$ attention heads to discover Fe$^{3+}$ diagnostic features from data, the same accuracy is obtained by initializing the bias from Table~\ref{tab:fe_diagnostics}.

The prior contributes a $+1.8$~percentage-point top-1 improvement over transformer-only at $H = 4$ ($0.791 \to 0.809$), but only $-0.7$~pp at $H = 8$ ($0.813 \to 0.806$). This is consistent with the standard observation that physics-informed priors help most when the model's capacity is constrained \cite{raissi2019pinn, karpatne2017tgds}: at $H = 8$ the model already discovers most of the diagnostic regions on its own (Figure~\ref{fig:trans4_attn} shows three of four heads doing so without any guidance at $H = 4$, and the picture is cleaner at $H = 8$), so the prior contributes nothing on top.

Two structural observations from training diagnostics support the capacity-saturation interpretation. First, the attention-bias RMS magnitude $\|\bm{\beta}\|_{\text{rms}}$ at the end of training was $0.977$ for the $H = 4$ manual run --- essentially unchanged from its initialization at $\alpha = 1.0$ --- but rose to $1.154$ for the $H = 8$ manual run, indicating that the eight-head model amplified the prior-anchored heads during training rather than discovering complementary features through its three additional random heads. Second, the average pairwise cosine similarity between heads' learned query vectors $\mathbf{q}_h$ was $0.015$ for $H = 8$ versus $0.031$ for $H = 4$, ruling out head collapse as the explanation: the random heads stayed distinct, but the model elected not to use that capacity for novel feature discovery.

\paragraph{Caveat on the legacy PCA-prior rows.}
\label{sec:pca_legacy_caveat}
The four PCA-prior rows in Table~\ref{tab:ablation} were produced by an earlier in-line PCA pipeline that, on closer inspection, fails to reproduce the spectroscopic intent of the prior: the bad-band-mask construction zeroed out 248 of 285 bands (every band where any single one of the 93 Group-1 reference spectra had a fill value), leaving only the 1500--1700~nm region with non-zero attention bias --- a region that does not contain any of the canonical Fe$^{3+}$ diagnostic wavelengths from Table~\ref{tab:fe_diagnostics}. The PCA configurations were therefore comparing an effectively-empty prior at the diagnostic wavelengths against transformer-only and manual-prior configurations whose biases sit exactly where they should. The reported PCA accuracies (0.804--0.821) should be interpreted as showing that the model recovers despite, rather than because of, the bad-band-masked PCA initialization. A re-curated PCA pipeline (\texttt{build\_group1\_ferric\_pca\_prior.py}) that filters the reference library to iron-bearing target/confuser/hard-negative classes, applies row z-score preprocessing, and runs PCA only on bands with sufficient cross-spectrum coverage, has been implemented to produce a corrected PCA bias that can be loaded via the \texttt{precomputed} mode of the training script (Section~\ref{chap:physics}). Re-running the PCA ablations with the corrected bias is left for future work.
