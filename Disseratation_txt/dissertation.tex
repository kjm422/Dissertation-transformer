...
In Part~II, $\mathbf{W}_p \in \R^{N \times L}$ denotes the full pixel-level reflectance matrix, while $\mathbf{w}_i \in \R^{L}$ denotes the spectrum of a single pixel $i$, with $w_{ij}$ the value at band $j$. The notation $\hat{w}$ indicates a normalized spectrum, $Q_1 = 95$ is the number of Group~1 mineral classes, and $c_{in}$ is the number of input features per band ($c_{in} = 3$ with spectral derivatives, $c_{in} = 1$ without).
...
This allows the model to recover per-band rescaling if the fixed normalization is suboptimal for particular wavelength regions.
...
An \emph{embedding} is the learned mapping that converts a raw input (a scalar reflectance value, a band index) into the higher-dimensional vector space in which the transformer operates. 
...
\end{equation}
Because reflectance is a continuous value rather than a discrete index, the content embedding is implemented as a matrix multiply rather than a table lookup, but both serve the same purpose: mapping raw inputs into the $d/2$-dimensional space in which the transformer operates.
...
and the model may benefit from learning which pairs of bands should be treated as ``nearby'' in embedding space independent of their physical wavelength separation.
...
The mixing matrix allows the model to learn cross-terms between content and position. 
For example, it learns that a reflectance drop (content) at band 150 (position, $\sim$1500~nm) has different significance than the same reflectance drop at band 50 ($\sim$750~nm). 
Layer normalization stabilizes the distribution of token embeddings across the sequence, which is important because the raw reflectance magnitudes vary by orders of magnitude across the VSWIR range.
...
The additive bias $\bm{\beta}_h \in \R^L$ is a learnable parameter that provides an interface for physics-informed priors (Chapter~\ref{chap:physics}).
...
The attention temperature $\tau_a \in (0, \infty)$ controls the sharpness of the attention distribution: $\tau_a < 1$ produces sharper (more selective) attention, $\tau_a > 1$ produces softer (more uniform) attention, and $\tau_a = 1$ recovers standard softmax.

\paragraph{Attention as pooling.}
The output $\mathbf{z}_h$ is a weighted average over all Value vectors, where the weights $\mathbf{A}_h$ sum to 1 across bands. 
This is precisely attention pooling.
...
The pooled representation $\mathbf{z}_{\text{cat}}$ is passed through a 2-layer feedforward classification head to produce class logits over $Q_1 = 94$ mineral classes. 
...
The first ReLU (Equation~\ref{eq:mlp_relu1}) operates directly on $\mathbf{z}_{\text{cat}}$ before any linear projection, providing a nonlinear gate that zeroes out negative activations from the concatenated head outputs. 
...
\begin{table}[ht]
\centering
\caption{Learnable parameter counts by component for $d = 192$, $L = 285$ bands, $Q_1 = 94$ classes, and $c_{in} = 3$ (with derivatives).}
\label{tab:params}
...
\label{eq:ce_loss}
\end{equation}
where $q_c \in \{0,1\}$ is the one-hot ground truth mineral class label, $\tilde{q}_c$ is the label-smoothed target, and $\hat{p}_c = \text{Softmax}(\text{logits})_c$ is the predicted class probability. Label smoothing ($\epsilon_{\text{ls}} = 0.05$ in the default configuration) prevents the model from becoming overconfident on training examples, which is important for a 94-class problem where some classes have few training samples and the model could otherwise memorize noisy labels.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Attention bias} $\bm{\beta}_h$: weight decay $= 0$, since the bias encodes band-importance information (either learned or physics-initialized) that should not be penalized toward zero.
\end{enumerate}
...
\label{eq:toa_decomp}
\end{equation}
where $T_{\downarrow}$ is the atmospheric transmittance along the sun-to-surface (downward) path, $T_{\uparrow}$ is the transmittance along the surface-to-sensor (upward) path, and $\rho_{\text{path}}$ is the path reflectance --- the atmosphere-only contribution from photons scattered into the sensor's field of view without ever reaching the surface \cite{richter_atcor_atbd,schaepman2006}. This is a first-order approximation that neglects the higher-order atmosphere--surface multiple-scattering term (the spherical albedo correction $1/(1 - S_{\text{atm}}\rho_s)$), which is small for the moderate-albedo arid surfaces that dominate EMIT's mineral targets.

The key insight is that Equation~\ref{eq:toa_decomp} has an additive--multiplicative structure: the surface signal is \emph{scaled} by atmospheric transmittance and \emph{offset} by path reflectance. In atmospheric windows --- spectral regions away from strong gas absorption --- transmittance is high ($T_{\downarrow} T_{\uparrow} \approx 0.7$--$0.95$) and varies smoothly with wavelength over the scale of narrow mineral absorption features. Path radiance decreases with wavelength due to Rayleigh scattering ($\propto \lambda^{-4}$) and is ``usually very small'' beyond 800~nm \cite{richter_atcor_atbd}. The diagnostic Fe$^{3+}$ features for hematite and goethite --- charge transfer at $\sim$530~nm, crystal field transitions at $\sim$670 and $\sim$860~nm \cite{clark1999,sherman1985,wang2022hyspex} --- reside in precisely these windows, so their positions and relative shapes are largely preserved from surface to TOA even though absolute magnitudes change.

This additive--multiplicative structure also directly motivates the LUSI predicates (Chapter~\ref{chap:lusi}): the scale predicate $T_{\text{scale}}$ (Equation~\ref{eq:scale_predicate}) models the multiplicative $T_{\downarrow} T_{\uparrow}$ transmittance variation due to topography and solar geometry, while the tilt predicate $T_{\text{tilt}}$ (Equation~\ref{eq:tilt_predicate}) models the additive, wavelength-dependent $\rho_{\text{path}}$ contribution from aerosol and Rayleigh scattering. By enforcing invariance to these two atmospheric perturbation modes, LUSI regularization encodes the physical structure of Equation~\ref{eq:toa_decomp} into the training objective.

The PCA eigenvectors from surface reflectance reference spectra therefore point toward the correct wavelength regions, even though they do not capture the TOA-specific variance from path radiance and aerosol slope \cite{richter_atcor_atbd}. This is the fundamental reason the RTM-bypass approach works: the spectral shape information that distinguishes minerals survives atmospheric propagation at diagnostic wavelengths. The problematic bands are where the atmosphere is opaque (water vapor at $\sim$1.4 and $\sim$1.9~$\mu$m), which is precisely why the water vapor mask (Section~\ref{sec:wv_mask}) is important.
...
The first principal component of the USGS library is the direction of maximum spectral variance \emph{across minerals}, which in practice is dominated by the wavelengths where minerals most disagree with each other. These are roughly the known diagnostic absorption bands (Fe$^{3+}$ at $\sim$530, 670, and 860~nm; clay OH near 2200~nm). Taking the absolute value of the eigenvector, $|\mathbf{v}_1|$, yields a broadband importance profile that emphasizes ``places where minerals differ from each other on average'' --- a data-driven summary of the reference library that points the model toward mineralogically informative wavelengths at initialization, rather than requiring the model to discover them from scratch. Subsequent components $\mathbf{v}_2, \mathbf{v}_3, \ldots$ capture progressively finer discriminative axes, allowing multiple attention heads to be seeded with complementary band-importance profiles.
...
The result is a modest accuracy improvement (Chapter~\ref{chap:ablation}).
...
This chapter describes a mechanism for injecting domain knowledge through the \emph{loss function}. Learning Using Statistical Invariants (LUSI) enforces that the model’s predictions are invariant to physical transformations that should not change mineral identity --- specifically, illumination scaling and atmospheric slope perturbation. LUSI can be used independently or in combination with the PCA priors of Chapter~\ref{chap:physics}. The physical motivation comes directly from the TOA decomposition (Equation~\ref{eq:toa_decomp}): since the atmosphere perturbs surface reflectance through a multiplicative transmittance factor ($T_{\downarrow} T_{\uparrow}$) and an additive path reflectance term ($\rho_{\text{path}}$), a robust mineral classifier should be invariant to both perturbation modes. The LUSI predicates encode exactly these two modes as data augmentations, and the consistency loss penalizes any change in the model’s predictions under these physically motivated transformations. This chapter presents the theoretical foundation, the specific physical predicates chosen for mineral classification, and the implementation details that ensure numerical stability.
...
Aerosol loading and Rayleigh scattering add a wavelength-dependent offset to TOA reflectance --- corresponding to the $\rho_{\text{path}}$ term in Equation~\ref{eq:toa_decomp} --- that is approximately linear in wavelength over the VSWIR range \cite{richter_atcor_atbd}. Path radiance decreases with wavelength ($\propto \lambda^{-4}$ for Rayleigh scattering), so its first-order effect on the spectrum is a slope. This additive tilt changes the continuum slope without affecting the position or depth of mineral absorption features. The tilt predicate applies a per-sample random linear slope:
\begin{equation}
T_{\text{tilt}}(\mathbf{w}) = \mathbf{w} + \xi \cdot \boldsymbol{\lambda}_{\text{grid}}, \quad \xi \sim \mathcal{N}(0, 0.01),
\label{eq:tilt_predicate}
\end{equation}
where $\boldsymbol{\lambda}_{\text{grid}} = \text{linspace}(-1, 1, L)$ is a normalized wavelength ramp.\footnote{The current implementation spaces $\boldsymbol{\lambda}_{\text{grid}}$ uniformly by band index rather than by physical wavelength. Because EMIT's band spacing is not perfectly uniform across the VSWIR range, the tilt perturbation per nanometer is slightly non-uniform. Given the small magnitude of $\xi$ ($\sigma = 0.01$), this approximation has negligible effect, but a future refinement could construct $\boldsymbol{\lambda}_{\text{grid}}$ from the actual EMIT wavelength centers.} A Gaussian distribution is chosen because atmospheric slope perturbations are centered on zero (no perturbation) with symmetric deviations in either direction. The standard deviation of 0.01 is conservative relative to the robust-normalized reflectance magnitudes, approximating the effect of moderate variations in aerosol optical depth without producing unphysical spectra.
...
\begin{equation}
\mathcal{L}_{\text{LUSI}} = D_{\text{KL}}(\mathbf{p}_{\text{trans}} \| \mathbf{p}_{\text{orig}}) \cdot \tau_l^2,
\label{eq:lusi_kl}
\end{equation}
where $D_{\text{KL}}(\mathbf{p} \| \mathbf{q}) = \sum_c p_c \log(p_c / q_c)$ measures how much the transformed prediction diverges from the original, and $\tau_l = 2.0$ is a temperature that softens the class distributions before comparison. The $\tau_l^2$ scaling follows the standard knowledge distillation convention \cite{hinton2015distilling}: higher temperature produces softer distributions where the KL divergence has smaller magnitude, so the quadratic scaling compensates to keep the gradient contribution approximately constant across temperature values. Following the teacher-student paradigm, gradients flow only through the student (transformed) path; the teacher (original) logits are stop-gradient. This trains the student to match the teacher’s prediction regardless of the physical perturbation.
